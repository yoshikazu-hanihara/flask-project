...
def calculate_raw_material_costs(inp, form):
    ...
    genzairyousyoukei_coefficient = (
        ...
        + ((mold_unit_price / mold_count) / MOLD_DIVISOR if include_kata else 0)
    ...
    # ここでは一例として売上(sales_price)で割る原材料費率を計算
...
def calculate_manufacturing_costs(inp, form, raw_material_cost_total):
    ...
    sales_price     = inp["sales_price"]  # 原価率の計算例で使用
    ...
    # ※本来は raw_dict["genzairyousyoukei_coefficient"] を加味して歩留まりを計算するケースがあるが、
    #   元コードが不完全なのであえてそのまま利用
    #   (yield_coefficient 先定義など、整合が取れていません)
    ...
    # 製造販管費合計 (本来は order_quantity を掛けるなど実装による)
